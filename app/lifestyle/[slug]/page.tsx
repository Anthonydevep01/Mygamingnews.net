...
export async function generateStaticParams() {
  ...
  return lifestyleArticles.map((article) => ({
    slug: article.slug,
  }))
}
...
export async function generateMetadata({ params }: ArticlePageProps) {
  ...
  if (!article || article.category.toLowerCase() !== 'lifestyle') {
    return {
  ...
  return {
    ...
    openGraph: {
      ...
      description: article.meta_description || article.description,
      url: `https://www.mygamingnews.net/lifestyle/${article.slug}`,
...
export default function LifestyleArticlePage({ params }: ArticlePageProps) {
  return (
    <ArticlePage 
      slug={params.slug} 
      category="Lifestyle" 
