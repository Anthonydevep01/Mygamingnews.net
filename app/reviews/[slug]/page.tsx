...
export async function generateStaticParams() {
  ...
  return reviewsArticles.map((article) => ({
    slug: article.slug,
  }))
}
...
export async function generateMetadata({ params }: ArticlePageProps) {
  ...
  if (!article || article.category.toLowerCase() !== 'reviews') {
    return {
  ...
  return {
    ...
    openGraph: {
      ...
      description: article.meta_description || article.description,
      url: `https://www.mygamingnews.net/reviews/${article.slug}`,
...
export default function ReviewsArticlePage({ params }: ArticlePageProps) {
  return (
    <ArticlePage 
      slug={params.slug} 
      category="Reviews" 
      categoryDisplayName="Reviews" 
    />
  )
}